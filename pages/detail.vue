  ...
  <div class="container">
    ...
    <div class="main">
      ...
      <div class="title-bottom">
        <nuxt-link :to="{path:`/search-result?time=${article.createdTime}`}">
...
export default {
  ...
  async asyncData() {
    let [article] = await Promise.all([
      axios.get("https://api.myjson.com/bins/nskn0")
    ]);
    return {
  ...
  methods: {
    getShareHref(item){
          ...
          if(item === 'facebook'){
            ...
            return `https://www.facebook.com/sharer.php?title=${this.article.title}&u=${encodeURIComponent(content)}`
          }
          if(item === 'twitter'){
            content+=`https://www.baidu.com?utm_source=twitter&utm_medium=organic&utm_campaign=share`;
            return `https://twitter.com/share?text=${this.article.title}&url=${encodeURIComponent(content)}`
          }
          if(item === 'whatsapp'){
                content+=this.article.title;
                content+="\n\n"+'https://www.baidu.com'+'?utm_source=whatsapp&utm_medium=organic&utm_campaign=share';
                return "whatsapp://send?text="+ encodeURIComponent(content);
          }
          if(item === 'line'){
          return`https://social-plugins.line.me/lineit/share?url=${encodeURIComponent('https://www.baidu.com')}`
          }
    },
    onClickLeft() {
      this.$router.go(-1);
